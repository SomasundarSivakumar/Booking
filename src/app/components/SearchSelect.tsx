...
export const SearchSelect = ({ 
    ...
    return (
        <div className="relative w-full" ref={wrapperRef}>
            ...
            {isOpen && (
                <div 
                    className={`absolute z-20 w-full bg-[#2D3E50] border border-[#ffffff1a] rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.5)] overflow-hidden animate-fade-in ${
                    ...
                    <ul className="max-h-48 overflow-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
                        {filteredOptions.length > 0 ? (
                            filteredOptions.map((option) => (
                                <li
                                    key={option.value}
                                    className={`p-4 text-sm cursor-pointer hover:bg-dynamic-orange hover:text-white transition-colors ${
